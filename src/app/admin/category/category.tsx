...
import { useFetchCategory } from "@/lib/category/useFetchCategories";
...
export default function Category() {
  ...
  const { successPromiseMessage, errorMessage, errorPromiseMessage } =
    useToast();
  const { data, error, loading, total, refresh } = useFetchCategory(filter);
  const deleted = useDeleteUser();
  const [isOpen, setIsOpen] = React.useState<boolean>(false);
  const [editOpen, setEditOpen] = React.useState<boolean>(false);
  const [searchTerm, setSearchTerm] = React.useState("");
  const [dataEvents, setDataEvents] = React.useState<ICategoriesTypes | null>(
    null
  );
  ...
  React.useEffect(() => {
    if (refreshAuto?.isStatus == "edit_category") {
      refresh();
      handleEditClose();
    }
  ...
  const handleDelete = async () => {
    try {
      const url = `categories/${dataEvents?.id}`;
      const result = await deleted.handleDelete(url);
      if (result.status === 200) {
        successPromiseMessage("Delete category success", 2000);
        refresh();
      } else {
        errorPromiseMessage("Delete category failed", 2000);
      }
      handleClose();
    } catch (error) {
  ...
  const handleSubmit = async (
    ...
    try {
      ...
      if (result?.status === 201) {
        successPromiseMessage(`Create category ${result}`, 2000);
        resetForm();
        refresh();
      } else if (result?.status === 409) {
        errorMessage("Category already created", 2000);
      } else {
        errorMessage("Create category failed", 2000);
      }
    } catch (error) {
      errorMessage("Create Category failed", 2000);
    }
  ...
  return (
    <>
      <div>
        <div className="py-2">
          <p className="text-base text-gray700">All Category</p>
          <div className="my-3 grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="lg:col-span-1 col-span-3">
              <div className="border border-gray300 rounded-sm my-3 py-5 px-4">
                <p className="text-base text-gray700">Create Category</p>
                <Formik
                  initialValues={{ name: "" }}
